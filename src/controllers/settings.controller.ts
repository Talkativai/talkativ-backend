...
// ─── Reconnect Phone ─────────────────────────────────────────────────────────
// Re-configures the business's Twilio number to point to the Ultravox inbound
// webhook. Fixes "application error" on existing numbers.
export const reconnectPhone = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!agent?.elevenlabsAgentId) throw ApiError.badRequest('No voice agent found — complete onboarding first');

  await elevenlabs.registerPhoneNumber(phoneConfig.assignedNumber, agent.elevenlabsAgentId);
